...
export const useMarketData = ({ marketAddress }: IUseMarketData) => {
  ...
  const market: Market | null = useMemo(
    () =>
      markets.find((market) => market.address[defaultChain.id]?.toLowerCase() == marketAddress?.toLowerCase()) ?? null,
    [marketAddress],
  );
  ...
  const { data: outcomeTokensPercent } = useQuery({
    queryKey: ['outcomeTokensPercent', fixedProductMarketMakerContract?.address, outcomeTokensBuyPrice],
    queryFn: async () => {
