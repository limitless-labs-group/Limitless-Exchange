    ...
    return markets?.filter((market) =>
      selectedFilterTokens.length > 0
        ? selectedFilterTokens.find(
            (filterToken) =>
              getAddress(filterToken.address[defaultChain.id]) === getAddress(market.collateralToken[defaultChain.id]),
          )
        : true,
    );
  ...
  return (
    <MainLayout maxContentWidth={'unset'}>
      <Stack w={'full'} spacing={5} px={{ md: 14 }}>
        ...
        <Grid templateColumns={{ sm: 'repeat(1, 1fr)', md: 'repeat(3, 1fr)', xl: 'repeat(4, 1fr)' }} gap={6}>
          <CreateMarketCard />
