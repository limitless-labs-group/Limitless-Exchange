import { collateralToken, collateralTokensArray, defaultChain, newSubgraphURI, onChain, weth } from '@/constants';
...
export const HistoryServiceProvider = ({ children }: PropsWithChildren) => {
  ...
  const { data: trades, refetch: getTrades } = useQuery({
    ...
    queryFn: async () => {
      ...
      _trades.map((trade) => {
        const outcomeTokenAmountBI = BigInt(trade.outcomeTokenAmounts.find((amount) => BigInt(amount) != 0n) ?? 0);
        ...
        trade.collateralAmount = formatUnits(BigInt(trade.outcomeTokenNetCost), collateralToken.decimals);
        trade.outcomeTokenPrice = (Number(trade.collateralAmount) / Number(trade.outcomeTokenAmount)).toString();

        // trade.outcomePercent = Number(trade.outcomeTokenPrice)
      });

      _trades.sort((tradeA, tradeB) => Number(tradeB.blockTimestamp) - Number(tradeA.blockTimestamp));
      console.log('trades', _trades);

      return _trades.filter((trade) => [weth.symbol, onChain.symbol].includes(trade.market.collateral?.symbol || ''));
    },
  ...
  const { data: redeems, refetch: getRedeems } = useQuery({
    ...
    queryFn: async () => {
      ...
      _redeems.sort((redeemA, redeemB) => Number(redeemB.blockTimestamp) - Number(redeemA.blockTimestamp));
    ...
    queryFn: async () => {
      ...
      trades?.forEach((trade) => {
        ...
        const existingMarket = _positions.find(
          (position) => position.market.id === trade.market.id && position.outcomeIndex === trade.outcomeIndex,
        );
      ...
      _positions = _positions.filter(
        (position) => !redeems?.find((redeem) => redeem.conditionId === position.market.condition_id),
      );
        ...
        _positions
          .map((position) => ({
            ...
            market: {
              ...position.market,
              collateral: {
                symbol: position.market.collateral?.symbol ? position.market.collateral?.symbol : 'MFER',
              },
            },
          }))
          // Todo remove this filter
          .filter((position) => [weth.symbol, onChain.symbol].includes(position.market.collateral.symbol))
      );
    },
  });
  ...
  const balanceInvested = useMemo(() => {
    ...
    positions?.forEach((position) => {
      ...
      const token = collateralTokensArray.find((token) => token.symbol === position.market.collateral?.symbol);
      if (!!token) {
  ...
  return <HistoryServiceContext.Provider value={contextProviderValue}>{children}</HistoryServiceContext.Provider>;
};
