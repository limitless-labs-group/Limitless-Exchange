...
export function useMarketByConditionId(conditionId: string) {
  const { data: market } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/markets/conditions/${conditionId}`);
      return response.data as Market;
    },
  });
...
export function useMarket(address: string) {
  const { data: market } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/markets/${address}`);
      return response.data as Market;
    },
  });
